...
function ChatViewContent(props: ChatViewProps) {
  ...
  const consumeComposerDraftContent = useComposerDraftStore(
    (store) => store.consumeComposerContent,
  );
  ...
  const onSend = async (
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      ...
      if (startResult._tag === "Failure") {
        ...
        consumeComposerDraftContent(composerDraftTarget, {
          prompt: promptForSend,
          imageIds: composerImagesSnapshot.map((image) => image.id),
          fileIds: composerFilesSnapshot.map((file) => file.id),
          terminalContextIds: composerTerminalContextsSnapshot.map((context) => context.id),
          elementContextIds: composerElementContextsSnapshot.map((context) => context.id),
          previewAnnotationIds: composerPreviewAnnotationsSnapshot.map(
            (annotation) => annotation.id,
          ),
          reviewCommentIds: composerReviewCommentsSnapshot.map((comment) => comment.id),
        });
        const remainingDraft =
          useComposerDraftStore.getState().getComposerDraft(composerDraftTarget) ?? null;
        const remainingPrompt = remainingDraft?.prompt ?? "";
        promptRef.current = remainingPrompt;
        composerImagesRef.current = remainingDraft?.images ?? [];
        composerFilesRef.current = remainingDraft?.files ?? [];
        composerTerminalContextsRef.current = remainingDraft?.terminalContexts ?? [];
        composerElementContextsRef.current = remainingDraft?.elementContexts ?? [];
        composerRef.current?.resetCursorState({
    ...
    if (failure !== null) {
      const draftPreviewUrls = new Set(
      ...
      setOptimisticUserMessages((existing) => {
        ...
        for (const message of removed) {
          revokeUserMessagePreviewUrlsExcept(message, draftPreviewUrls);
        }
