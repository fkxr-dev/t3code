...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "T3 Code (Pi)",
    linuxWmClass: "t3code-pi",
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "MimeType=x-scheme-handler/t3code-pi;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "t3code-pi",
      desktopEntryPath: "/home/alice/.local/share/applications/t3code-pi-url-handler.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the t3code-pi:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/t3code-pi-url-handler.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the t3code-pi:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "t3code-pi-url-handler.desktop", "x-scheme-handler/t3code-pi"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/t3code-pi-url-handler.desktop",
        }),
      });
