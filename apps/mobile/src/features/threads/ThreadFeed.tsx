...
function MessageAttachmentFile(props: {
  ...
  return (
    ...
    >
      ...
      <Text className="text-xs text-foreground-muted">{sizeLabel}</Text>
    </Pressable>
  );
}

function MessageAttachmentUnsupported(props: { readonly name: string }) {
  return (
    <View
      accessible
      accessibilityRole="text"
      accessibilityLabel={`Unsupported attachment ${props.name}`}
      className="flex-row items-center gap-2 py-1"
    >
      <SymbolView
        name="doc.text"
          ...
          >
            ...
            {attachments.map((attachment) => {
              return attachment.type === "image" ? (
                ...
                <MessageAttachmentUnsupported key={attachment.id} name={attachment.name} />
              );
            })}
          </View>
