...
export const withPartialClaimCleanup = <A, E, R>(
  effect: Effect.Effect<A, E, R>,
  claimedPaths: ReadonlyArray<string>,
) =>
  effect.pipe(
    Effect.onExit((exit) =>
      Exit.isSuccess(exit) ? Effect.void : releaseClaimedAttachments(claimedPaths),
    ),
  );
...
export const claimPendingAttachments = Effect.fn("AttachmentClaims.claimPendingAttachments")(
  function* (input: {
    ...
    const attachments = yield* withPartialClaimCleanup(
      Effect.forEach(
        input.attachments,
        (attachment) =>
          Effect.gen(function* () {
            ...
            const finalExists = yield* fileSystem.exists(claim.finalPath).pipe(
              Effect.mapError(
                (cause) =>
                  new AttachmentClaimError({
                    message: `Failed to inspect attachment '${attachment.name}' for this thread.`,
                    cause,
                  }),
              ),
            );
            if (finalExists) {
              const finalInfo = yield* fileSystem.stat(claim.finalPath).pipe(
                Effect.mapError(
                  (cause) =>
                    new AttachmentClaimError({
                      message: `Failed to inspect attachment '${attachment.name}' for this thread.`,
              ...
              if (Number(finalInfo.size) !== attachment.sizeBytes) {
                return yield* new AttachmentClaimError({
                  message: `Attachment '${attachment.name}' cannot be sent: claimed copy size does not match.`,
                });
              }
              return normalized;
            }
