...
describe("AttachmentClaims", () => {
  ...
  it.effect("does not make an existing deterministic claim eligible for rollback", () =>
    Effect.gen(function* () {
      const pending = yield* stagePendingUpload({
      ...
      const firstClaim = yield* claimPendingAttachments({
        threadId: "thread-claims-existing",
        attachments: [pending],
      });
      const finalPath = firstClaim.claimedPaths[0]!;
      const originalContents = NodeFS.readFileSync(finalPath);

      const repeatedClaim = yield* claimPendingAttachments({
        threadId: "thread-claims-existing",
        attachments: [pending],
      });

      expect(repeatedClaim.attachments).toEqual(firstClaim.attachments);
      expect(repeatedClaim.claimedPaths).toEqual([]);
      yield* releaseClaimedAttachments(repeatedClaim.claimedPaths);
      expect(NodeFS.readFileSync(finalPath)).toEqual(originalContents);
    }).pipe(Effect.provide(testLayer)),
  );

  it.effect("releases partial claims when claiming is interrupted", () =>
    Effect.gen(function* () {
      const pending = yield* stagePendingUpload({
      ...
      const claimed = yield* claimPendingAttachments({
        threadId: "thread-claims-interrupted",
        attachments: [pending],
      });
      expect(NodeFS.existsSync(claimed.claimedPaths[0]!)).toBe(true);

      const claimFiber = yield* Effect.forkChild(
        withPartialClaimCleanup(Effect.never, claimed.claimedPaths),
