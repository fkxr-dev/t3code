...
describe("ClaudeAdapterV2 attachments", () => {
  it.effect("forwards images natively and generic files by path", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const attachmentPath = attachmentRelativePath(attachment);
        if (attachmentPath === null) {
          throw new Error("Expected a persisted image attachment path.");
        }
        yield* fileSystem.writeFile(
          path.join(attachmentsDir, attachmentPath),
          Uint8Array.from([1, 2, 3, 4]),
        );
        const fileAttachment = ChatFileAttachment.make({
          type: "file",
          id: ChatAttachmentId.make(
            "thread-claude-attachments-12345678-1234-1234-1234-123456789abd-pdf",
          ),
          name: "report.pdf",
          mimeType: "application/pdf",
          sizeBytes: 3,
        });
        const fileAttachmentPath = attachmentRelativePath(fileAttachment);
        if (fileAttachmentPath === null) {
          throw new Error("Expected a persisted file attachment path.");
        }
        yield* fileSystem.writeFile(
          path.join(attachmentsDir, fileAttachmentPath),
        ...
        const expectedAttachmentPath = path.join(attachmentsDir, attachmentPath);
        const expectedFileAttachmentPath = path.join(attachmentsDir, fileAttachmentPath);
        assert.deepEqual(offeredMessages[0]?.message.content, [
          {
            type: "text",
            text: `Ultrathink:\nWhat's in this image?\n\n[Attached image "diagram.png" is saved at: ${expectedAttachmentPath}]\n[Attached file "report.pdf" is saved at: ${expectedFileAttachmentPath}]`,
          },
          expectedImageBlock,
        ]);
