  ...
  return yield* Effect.forEach(
    ...
    Effect.fn("ws.assets.persistChatAttachment")(function* ({ attachment, index }) {
      ...
      if (relativePath === null) {
        return yield* new PersistChatAttachmentsError({
          message: `Could not resolve attachment path for ${attachment.name}.`,
        });
      }
...
const makeWsRpcLayer = (
  ...
  ServerWsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const attachmentFileSystem = yield* FileSystem.FileSystem;
            ...
            startup
              .enqueueCommand(
                Effect.uninterruptibleMask((restore) =>
                  Effect.gen(function* () {
                    // Claim inside the queued work so a disconnected RPC client
                    // cannot cancel its wait and roll back a command that still runs.
                    const claimed =
                    ...
                    return yield* threadManagement.dispatch(
                  ...
                  Effect.provideService(FileSystem.FileSystem, attachmentFileSystem),
                ),
              )
              .pipe(
          ...
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              return yield* startup
                ...
                .pipe(
                  Effect.mapError((cause) =>
                    Schema.is(OrchestrationV2ThreadLaunchError)(cause)
                      ? cause
                      : new OrchestrationV2ThreadLaunchError({
                          commandId: input.commandId,
                          projectId: input.projectId,
